...
from telegram_bot.state.state import UnbanUser  # Предположим, что у вас есть состояние UnbanUser
...
async def unban_user(user_id: int, session) -> dict:
    ...
    try:
        ...
        logger.error('Произошла ошибка при разблокировке пользователя', e)
