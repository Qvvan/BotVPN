...
class MessageLoggingMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, Message) and not event.from_user.is_bot:
            ...
            if throttling_middleware and not await throttling_middleware.is_throttled(event):
                sent_message = await event.answer("Загрузка 🔄", reply_markup=ReplyKeyboardRemove())
