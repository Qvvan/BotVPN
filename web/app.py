...
async def get_key(encrypted_part: str, request: Request, db: Session = Depends(get_db)):
    ...
    for server in servers:
        ...
        await outline_manager.delete_key(encrypted_part[85:])
    ...
    if eligible_servers:
        ...
        server_keys_info.sort(key=lambda x: x[1])
        selected_server = server_keys_info[0][0]
    ...
    new_key = await outline_manager.create_key(str(user_id), encrypted_part[85:])
